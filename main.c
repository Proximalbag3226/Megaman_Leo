...
int main() {
    ...
    sp = initSerialPort("COM3", B9600);
    if (!isConnected(&sp)) {
        printf("No se pudo conectar al Arduino en COM3\n");
    } else {
        printf("Conectado al Arduino\n");
    }
...
void Etrada_teclas() {
   ...
   /*if (isConnected(&sp)) {
        char buffer[256];
        if (readSerialPortUntilEndLine(buffer, sizeof(buffer), &sp) > 0) {
            int ax, ay, aA, aB, aC, aD;
            if (sscanf(buffer, "%d,%d,%d,%d,%d,%d", &ax, &ay, &aA, &aB, &aC, &aD) == 6) {
                controles.izquierda = (ax < 400);
                controles.derecha = (ax > 600);
                controles.arriba = aA;
                controles.abajo = aC;
                controles.salto = aD;

                if (aB && !prev_B && !mega.atacando) {
                    mega.atacando = true;
                    mega.anim_frame = 0;
                    mega.timer_anim = 0;
                    disparar();
...
void Fisicas() {
    ...
    Rect rMegaX = {(int)mega.x, (int)mega.y, 50, 50};
    for(int i = 0; i < num_plataformas; i++) {
        if (choco(rMegaX, plataformas[i])) {
            if (mega.vx > 0) {
                mega.x = plataformas[i].x - 50;
            } else if (mega.vx < 0) {
                mega.x = plataformas[i].x + plataformas[i].w;
            }
            rMegaX.x = (int)mega.x;
        }
    }
    ...
    Rect rMegaY = {(int)mega.x, (int)mega.y, 50, 50};
    for(int i = 0; i < num_plataformas; i++) {
        if (choco(rMegaY, plataformas[i])) {
            if (mega.vy > 0) {
                ...
                mega.y = plataformas[i].y + plataformas[i].h;
                mega.vy = 0;
            }
            rMegaY.y = (int)mega.y;
        }
...
void actualizarEnemigos() {
    for (int i = 0; i < MAX_ENEMIGOS; i++) {
        ...
        Rect rEnemigoX = {(int)enemigos[i].x, (int)enemigos[i].y, enemigos[i].w, enemigos[i].h};
        for (int j = 0; j < num_plataformas; j++) {
            if (choco(rEnemigoX, plataformas[j])) {
                if (enemigos[i].vx > 0) enemigos[i].x = plataformas[j].x - enemigos[i].w;
                else if (enemigos[i].vx < 0) enemigos[i].x = plataformas[j].x + plataformas[j].w;

                if (enemigos[i].estado_ia == 0) enemigos[i].direccion *= -1;
                break;
            }
        }
        ...
        Rect rEnemigoY = {(int)enemigos[i].x, (int)enemigos[i].y, enemigos[i].w, enemigos[i].h};
        for (int j = 0; j < num_plataformas; j++) {
            if (choco(rEnemigoY, plataformas[j])) {
                if (enemigos[i].vy > 0) {
                    enemigos[i].y = plataformas[j].y - enemigos[i].h;
                    enemigos[i].vy = 0;
                } else if (enemigos[i].vy < 0) {
                    enemigos[i].y = plataformas[j].y + plataformas[j].h;
                    enemigos[i].vy = 0;
                }
                break;
            }
        ...
        if (enemigos[i].estado_ia == 1 && enemigos[i].anim_frame == 3 && enemigos[i].timer_anim == 0) {
            enemigoDisparar(&enemigos[i]);
        }

        if (enemigos[i].cooldown_disparo > 0) enemigos[i].cooldown_disparo--;
        enemigos[i].hitbox = (Rect){(int)enemigos[i].x, (int)enemigos[i].y, enemigos[i].w, enemigos[i].h};
    }
}
...
void dibujarJuego() {
    ...
    if (!mega.herido) {
        ...
        if(sprite_jugador) {
            ...
            ventana.muestraImagenEscalada(dx, dy, 50, 50, spr_idle_R);
        }
    }
...
void cargar() {
    ...
    for (int i=10; i<17; i++){
        sprintf(path, "metal_disparo_%d.bmp", i);
        sprintf(mask, "metal_disparo_%d_mask.bmp", i);
        spr_e_atk_R[i-10] = ventana.creaImagenConMascara(path, mask);
    }

    for (int i=0; i<FRAMES_E_ATK; i++){
        sprintf(path, "metal_disparo_%d.bmp", i);
        sprintf(mask, "metal_disparo_%d_mask.bmp", i);
        spr_e_atk_L[i] = ventana.creaImagenConMascara(path, mask);
    }
